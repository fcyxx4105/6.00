...
def makeHist(data, bins, title, xLabel, yLabel):
    ...
    pylab.annotate('Mean =' + str(round(mean, 2)) +\
              ...
              xy = (0.54, 0.75), xycoords = 'axes fraction')
    pylab.show()
...
pylab.xlabel('Samole Number')
pylab.ylabel('Sample Mean')
pylab.title('Means of Samples os Size ' + str(4))
pylab.legend()
...
#      'of true mean of right dist. =', area)
...
#makeHist(times, 20, '2012 Boston Maraton',
#            'Minutes to Conplete Race', 'Number of Runners')
#sampleSize = 40
#sampleTimes(times, sampleSize)
"""
mean, var = plotMeans(1, 100000, 11, '1 die', 'w', '*')
print('Mean of rolling 1 dies=', round(mean, 4),
      'Variance =', round(var, 4))
mean, varf = plotMeans(100, 100000, 11,
                       'Mean of 100 dice', 'w', '//')
